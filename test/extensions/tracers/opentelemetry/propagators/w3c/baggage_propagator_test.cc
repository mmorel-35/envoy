...
TEST_F(BaggagePropagatorTest, NameReturnsBaggage) { 
  EXPECT_EQ(propagator_->name(), "baggage"); 
}
...
TEST_F(BaggagePropagatorTest, ParseValidBaggageEntries) {
  ...
  Tracing::TestTraceContextImpl trace_context{{"baggage", "userId=alice,serverNode=DF28,isProduction=true"}};
...
TEST_F(BaggagePropagatorTest, ParseBaggageWithProperties) {
  ...
  Tracing::TestTraceContextImpl trace_context{{"baggage", "userId=alice;property=value,serverNode=DF28"}};
