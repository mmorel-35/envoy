...
TEST_F(OpenTelemetryDriverTest, ParseSpanContextFromHeadersTest) {
  ...
  request_headers.set(TraceContextConstants::get().TRACE_PARENT.key(), parent_trace_header);
  // Also add tracestate.
  request_headers.set(TraceContextConstants::get().TRACE_STATE.key(), "test=foo");
  ...
  request_headers.remove(TraceContextConstants::get().TRACE_PARENT.key());
  request_headers.remove(TraceContextConstants::get().TRACE_STATE.key());
  span->injectContext(request_headers, Tracing::UpstreamContext());

  auto sampled_entry = request_headers.get(TraceContextConstants::get().TRACE_PARENT.key());
  EXPECT_EQ(sampled_entry.has_value(), true);
  EXPECT_EQ(
      sampled_entry.value(),
      absl::StrJoin({version, trace_id_hex, Hex::uint64ToHex(new_span_id), trace_flags}, "-"));

  auto sampled_tracestate_entry = request_headers.get(TraceContextConstants::get().TRACE_STATE.key());
...
TEST_F(OpenTelemetryDriverTest, NullSpanWithPropagationHeaderError) {
  ...
  Tracing::TestTraceContextImpl request_headers{
      {":authority", "test.com"}, {":path", "/"}, {":method", "GET"}};
  request_headers.set(TraceContextConstants::get().TRACE_PARENT.key(), "invalid00-0000000000000003-01");
