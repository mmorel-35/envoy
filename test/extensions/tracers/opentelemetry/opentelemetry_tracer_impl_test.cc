...
TEST_F(OpenTelemetryDriverTest, ParseSpanContextFromHeadersTest) {
  ...
  request_headers.set(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_PARENT.key(), parent_trace_header);
  // Also add tracestate.
  request_headers.set(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_STATE.key(), "test=foo");
  ...
  request_headers.remove(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_PARENT.key());
  request_headers.remove(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_STATE.key());
  span->injectContext(request_headers, Tracing::UpstreamContext());

  auto sampled_entry = request_headers.get(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_PARENT.key());
  ...
  auto sampled_tracestate_entry =
      request_headers.get(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_STATE.key());
...
TEST_F(OpenTelemetryDriverTest, NullSpanWithPropagationHeaderError) {
  ...
  Tracing::TestTraceContextImpl request_headers{
      {":authority", "test.com"}, {":path", "/"}, {":method", "GET"}};
  request_headers.set(Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_PARENT.key(),
