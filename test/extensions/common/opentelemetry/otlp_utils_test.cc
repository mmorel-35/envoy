...
TEST(OtlpUtilsTest, GetOtlpUserAgentHeader) {
  const auto& header = OtlpUtils::getOtlpUserAgentHeader();
  ...
  EXPECT_EQ(&header, &OtlpUtils::getOtlpUserAgentHeader());
}

TEST(OtlpUtilsTest, GetStringKeyValueSetsKeyAndStringValue) {
  auto kv = OtlpUtils::getStringKeyValue("my_key", "my_value");
  EXPECT_EQ("my_key", kv.key());
  EXPECT_EQ("my_value", kv.value().string_value());
}

TEST(OtlpUtilsTest, GetStringKeyValueEmptyStrings) {
  auto kv = OtlpUtils::getStringKeyValue("", "");
  EXPECT_EQ("", kv.key());
  EXPECT_EQ("", kv.value().string_value());
}

TEST(OtlpUtilsTest, PopulateAnyValueBool) {
  AnyValue proto;
  OTelAttribute attr = true;
  OtlpUtils::populateAnyValue(proto, attr);
  EXPECT_TRUE(proto.bool_value());
}

TEST(OtlpUtilsTest, PopulateAnyValueInt32) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueInt64) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueUInt32) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueUInt64) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueDouble) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueString) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueStringView) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueVectorString) {
  AnyValue proto;
...
TEST(OtlpUtilsTest, PopulateAnyValueVectorStringView) {
  AnyValue proto;
