...
TEST_F(BaggagePropagatorTest, ExtractDoesNotProvideTraceContext) {
  ...
  EXPECT_THAT(result.status().message(), HasSubstr("Baggage propagator does not extract trace context"));
}

TEST_F(BaggagePropagatorTest, InjectPreservesExistingBaggage) {
  Tracers::OpenTelemetry::SpanContext span_context("00", "0af7651916cd43dd8448eb211c80319c", "b7ad6b7169203331", true, "");
...
TEST_F(BaggagePropagatorTest, FormatBaggage) {
  ...
  EXPECT_THAT(result, testing::AnyOf(
    testing::Eq("key1=value1,key2=value2"),
...
TEST_F(BaggagePropagatorTest, Name) {
  EXPECT_EQ("baggage", propagator_->name());
}
