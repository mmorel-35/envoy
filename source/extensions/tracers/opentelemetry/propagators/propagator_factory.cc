...
CompositePropagatorPtr PropagatorFactory::createPropagators(const std::vector<std::string>& propagator_names,
  ...
  END_TRY catch (const EnvoyException& e) {
    ENVOY_LOG(debug, "Failed to read OTEL_PROPAGATORS environment variable: {}. Using defaults.", e.what());
  }
  ...
  ENVOY_LOG(debug, "No propagators specified in config or OTEL_PROPAGATORS, using default W3C Trace Context");
