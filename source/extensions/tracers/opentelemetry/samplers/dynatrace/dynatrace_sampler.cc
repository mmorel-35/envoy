...
void addSamplingAttributes(
    uint32_t sampling_exponent,
    Envoy::Extensions::OpenTelemetry::Sdk::Common::OTelAttributes& attributes) {
...
SamplingResult DynatraceSampler::shouldSample(
  ...
  if (!att.empty()) {
    result.attributes =
        std::make_unique<const Envoy::Extensions::OpenTelemetry::Sdk::Common::OTelAttributes>(
            std::move(att));
  }
