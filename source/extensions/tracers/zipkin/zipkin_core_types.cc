...
void Span::injectContext(Tracing::TraceContext& trace_context, const Tracing::UpstreamContext&) {
  ...
  MultiConstants::get().X_B3_TRACE_ID.setRefKey(trace_context, traceIdAsHexString());
  MultiConstants::get().X_B3_SPAN_ID.setRefKey(trace_context, idAsHexString());

  // Set the parent-span header properly, based on the newly-created span structure.
  if (isSetParentId()) {
    MultiConstants::get().X_B3_PARENT_SPAN_ID.setRefKey(trace_context, parentIdAsHexString());
  }

  // Set the sampled header.
  MultiConstants::get().X_B3_SAMPLED.setRefKey(trace_context, sampled() ? SAMPLED : NOT_SAMPLED);
