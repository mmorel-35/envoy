...
bool SpanContextExtractor::propagationHeaderPresent() {
  auto propagation_header = Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_PARENT.get(trace_context_);
...
absl::StatusOr<SpanContext> SpanContextExtractor::extractSpanContext() {
  ...
  const auto tracestate_values = Envoy::Extensions::Propagators::W3c::Constants::get().TRACE_STATE.getAll(trace_context_);
