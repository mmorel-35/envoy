...
BaggagePropagator::parseBaggage(absl::string_view baggage_header) {
  ...
  for (absl::string_view entry : entries) {
    ...
    if (!decoded_key.empty() && !decoded_value.empty() && 
        isValidBaggageKey(decoded_key) && isValidBaggageValue(decoded_value)) {
...
std::string BaggagePropagator::urlDecode(absl::string_view input) {
  ...
  for (size_t i = 0; i < input.size(); ++i) {
    if (input[i] == '%' && i + 2 < input.size()) {
      ...
      if (hex_str.size() == 2 && absl::ascii_isxdigit(hex_str[0]) && absl::ascii_isxdigit(hex_str[1])) {
