...
GenericCompositePropagator::GenericCompositePropagator(std::vector<GenericPropagatorPtr> propagators)
    : propagators_(std::move(propagators)) {}

absl::StatusOr<SpanContext> GenericCompositePropagator::extract(const Tracing::TraceContext& trace_context) {
  for (const auto& propagator : propagators_) {
    auto result = propagator->extract(trace_context);
    if (result.ok()) {
      ENVOY_LOG(trace, "Successfully extracted span context using {} propagator", propagator->name());
      return result;
    }
...
void GenericCompositePropagator::inject(const SpanContext& span_context, Tracing::TraceContext& trace_context) {
  for (const auto& propagator : propagators_) {
...
absl::StatusOr<Baggage> GenericCompositePropagator::extractBaggage(const Tracing::TraceContext& trace_context) {
  for (const auto& propagator : propagators_) {
    ...
    ENVOY_LOG(trace, "Failed to extract baggage using {} propagator: {}", 
              propagator->name(), result.status().message());
  }
...
void GenericCompositePropagator::injectBaggage(const Baggage& baggage, Tracing::TraceContext& trace_context) {
  for (const auto& propagator : propagators_) {
...
bool GenericCompositePropagator::propagationHeaderPresent(const Tracing::TraceContext& trace_context) {
  for (const auto& propagator : propagators_) {
    for (const auto& field : propagator->fields()) {
      if (trace_context.getByKey(field).has_value()) {
        ENVOY_LOG(trace, "Found propagation header '{}' for {} propagator", field, propagator->name());
        return true;
      }
    }
  }
