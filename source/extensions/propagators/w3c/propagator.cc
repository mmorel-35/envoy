 ...
 */
void injectTracestateIfPresent(const TraceContext& w3c_context, Tracing::TraceContext& trace_context) {
 ...
 */
TraceParent createChildTraceparent(const TraceContext& parent_context, absl::string_view new_span_id) {
  return TraceParent(
 ...
 */
TraceParent createRootTraceparent(absl::string_view trace_id, absl::string_view span_id, bool sampled) {
 ...
 */
bool updateBaggageWithKeyValue(Tracing::TraceContext& trace_context, 
 ...
 */
std::map<std::string, std::string> extractAllBaggageAsMap(const Tracing::TraceContext& trace_context) {
