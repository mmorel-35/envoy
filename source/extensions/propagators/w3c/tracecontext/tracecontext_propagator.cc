...
bool hasValidComponentSizes(absl::string_view version, absl::string_view trace_id,
  ...
  return version.size() == Constants::kVersionSize && trace_id.size() == Constants::kTraceIdSize &&
...
bool hasValidHexComponents(absl::string_view version, absl::string_view trace_id,
                           absl::string_view span_id, absl::string_view trace_flags) {
  return isValidHex(version) && isValidHex(trace_id) && isValidHex(span_id) &&
...
TraceContextPropagator::extractTraceParent(const Tracing::TraceContext& trace_context) const {
  auto header_value = TraceContextConstants::get().TRACE_PARENT.get(trace_context);
  return header_value;
}

absl::optional<std::string>
TraceContextPropagator::extractTraceState(const Tracing::TraceContext& trace_context) const {
...
absl::StatusOr<TraceParentInfo>
TraceContextPropagator::parseTraceParent(absl::string_view traceparent_value) const {

  if (traceparent_value.size() != Constants::kTraceparentHeaderSize) {
    return absl::InvalidArgumentError(absl::StrCat("Invalid traceparent header length: expected ",
                                                   Constants::kTraceparentHeaderSize, ", got ",
                                                   traceparent_value.size()));
  }
  ...
  if (!hasValidComponentSizes(version, trace_id, span_id, trace_flags)) {
    return absl::InvalidArgumentError(absl::StrCat(
        "Invalid traceparent field sizes: version=", version.size(), ", trace_id=", trace_id.size(),
        ", span_id=", span_id.size(), ", trace_flags=", trace_flags.size()));
  }
  ...
  if (isAllZeros(trace_id)) {
    return absl::InvalidArgumentError(
        "Invalid trace_id: cannot be all zeros per W3C specification");
  }
  ...
  return TraceParentInfo{std::string(version), std::string(trace_id), std::string(span_id),
                         sampled};
}

void TraceContextPropagator::injectTraceParent(Tracing::TraceContext& trace_context,
                                               absl::string_view version,
                                               absl::string_view trace_id,
                                               absl::string_view span_id, bool sampled) const {
  ...
  std::string traceparent_value =
      absl::StrCat(version, "-", trace_id, "-", span_id, "-", trace_flags_hex);

  TraceContextConstants::get().TRACE_PARENT.setRefKey(trace_context, traceparent_value);
}

void TraceContextPropagator::injectTraceState(Tracing::TraceContext& trace_context,
