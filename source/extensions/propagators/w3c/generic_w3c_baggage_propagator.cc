...
GenericW3CBaggagePropagator::GenericW3CBaggagePropagator()
    : baggage_header_("baggage") {}

absl::StatusOr<SpanContext> GenericW3CBaggagePropagator::extract(const Tracing::TraceContext&) {
...
void GenericW3CBaggagePropagator::inject(const SpanContext&, Tracing::TraceContext&) {
  // Baggage propagator doesn't inject span context, only baggage
}

absl::StatusOr<Baggage> GenericW3CBaggagePropagator::extractBaggage(const Tracing::TraceContext& trace_context) {
...
void GenericW3CBaggagePropagator::injectBaggage(const Baggage& baggage, Tracing::TraceContext& trace_context) {
  if (baggage.empty()) {
    return;
  }
...
std::vector<std::string> GenericW3CBaggagePropagator::fields() const {
  return {"baggage"};
}

std::string GenericW3CBaggagePropagator::name() const {
  return "baggage";
}

absl::StatusOr<Baggage> GenericW3CBaggagePropagator::parseBaggage(const std::string& baggage_value) {
  Baggage baggage;
...
std::string GenericW3CBaggagePropagator::formatBaggage(const Baggage& baggage) {
