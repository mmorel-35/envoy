# W3C Propagators Usage Examples

This document demonstrates how to use the W3C TraceContext and Baggage propagators.
...
using namespace Envoy::Extensions::Propagators::W3c::TraceContext;
...
propagator.injectTraceParent(outgoing_headers, "00", 
...
using namespace Envoy::Extensions::Propagators::W3c::Baggage;
...
void injectContext(Tracing::TraceContext& trace_context, 
  ...
  trace_propagator.injectTraceParent(trace_context, 
                                   kDefaultVersion, 
                                   getTraceId(), 
                                   getSpanId(), 
                                   sampled());
...
public:
  ...
  void injectIntoOutgoingRequest(Http::RequestHeaderMap& headers) {
    ...
    trace_propagator_.injectTraceParent(trace_context, "00",
                                      getCurrentTraceId(),
                                      getCurrentSpanId(),
                                      isSampled());
...
1. **Reuse Propagator Instances**: Propagators are stateless and can be reused
