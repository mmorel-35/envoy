...
absl::StatusOr<Baggage> BaggagePropagator::extractBaggage(const Tracing::TraceContext& trace_context) {
...
void BaggagePropagator::injectBaggage(const Baggage& baggage, Tracing::TraceContext& trace_context) {
  if (baggage.empty()) {
    return;
  }
