...
std::vector<std::string> B3Propagator::fields() const {
  return {
    std::string(B3_SINGLE_HEADER),
    std::string(B3_TRACE_ID_HEADER), 
    std::string(B3_SPAN_ID_HEADER),
    std::string(B3_PARENT_SPAN_ID_HEADER), 
    std::string(B3_SAMPLED_HEADER),
    std::string(B3_FLAGS_HEADER)
  };
}
...
B3Propagator::extractSingleHeader(const Tracing::TraceContext& trace_context) {
  ...
  if (!Extensions::Tracers::Zipkin::Util::parseTraceId(std::string(parts[0]), trace_id_high, trace_id_low)) {
    return absl::InvalidArgumentError("Invalid trace ID in B3 header");
  }

  // Parse span ID (parts[1])
  uint64_t span_id = 0;
  if (!Extensions::Tracers::Zipkin::Util::parseSpanId(std::string(parts[1]), span_id)) {
  ...
  if (parts.size() > 3) {
    if (!Extensions::Tracers::Zipkin::Util::parseSpanId(std::string(parts[3]), parent_span_id)) {
      return absl::InvalidArgumentError("Invalid parent span ID in B3 header");
    }
  }

  return Extensions::Tracers::Zipkin::SpanContext(trace_id_high, trace_id_low, span_id, parent_span_id, sampled);
}
...
B3Propagator::extractMultiHeader(const Tracing::TraceContext& trace_context) {
  ...
  // Parse trace ID
  uint64_t trace_id_high = 0, trace_id_low = 0;
  if (!Extensions::Tracers::Zipkin::Util::parseTraceId(trace_id_header.value(), trace_id_high, trace_id_low)) {
    return absl::InvalidArgumentError("Invalid trace ID in B3 headers");
  }

  // Parse span ID
  uint64_t span_id = 0;
  if (!Extensions::Tracers::Zipkin::Util::parseSpanId(span_id_header.value(), span_id)) {
  ...
  if (parent_span_id_header.has_value()) {
    if (!Extensions::Tracers::Zipkin::Util::parseSpanId(parent_span_id_header.value(), parent_span_id)) {
...
void B3Propagator::injectSingleHeader(const Extensions::Tracers::Zipkin::SpanContext& span_context,
                                      Tracing::TraceContext& trace_context) {
  std::string b3_value = Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.traceIdHigh()) +
                         Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.traceId()) + "-" +
                         Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.id());
  ...
  if (span_context.parentId() != 0) {
    b3_value += "-" + Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.parentId());
  }
...
void B3Propagator::injectMultiHeader(const Extensions::Tracers::Zipkin::SpanContext& span_context,
  ...
  std::string trace_id = Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.traceIdHigh()) +
                         Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.traceId());
  trace_context.setByKey(B3_TRACE_ID_HEADER, trace_id);

  // Inject span ID
  trace_context.setByKey(B3_SPAN_ID_HEADER, Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.id()));

  // Inject parent span ID if present
  if (span_context.parentId() != 0) {
    trace_context.setByKey(B3_PARENT_SPAN_ID_HEADER, 
                           Extensions::Tracers::Zipkin::Util::uint64ToHexString(span_context.parentId()));
  }
